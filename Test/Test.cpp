...
inline int FaceDegree(const std::vector<uint32_t> &p) {
    return p.size();
}

inline int FindPos(const std::vector<uint32_t> &p, uint32_t val) {
...
// dest.polygons[f0] is rotated by `seedRot` relative to base.polygons[f0].
...
inline PropagateResult TryPropagate(
    ...
    std::vector<char> queuedOrResolved(base.polygons.size(), 0);
    ...
    while (!q.empty()) {
        ...
        int n = FaceDegree(base.polygons[f]);
        int dn = FaceDegree(dest.polygons[f]);
        ...
        for (int k = 0; k < n; ++k) {
            uint32_t bv = base.polygons[f][k];
            uint32_t dv = dest.polygons[f][(k + rot) % n];
        ...
        for (int k = 0; k < n; ++k) {
            ...
            for (uint32_t f2 : vertFaces[bv]) {
                ...
                int k2 = FindPos(base.polygons[f2], bv);
                if (k2 < 0) continue;  // shouldn't happen

                int dn2 = FaceDegree(dest.polygons[f2]);
                int64_t dv = getCorr(bv);
                int p2 = FindPos(dest.polygons[f2], (uint32_t)dv);
...
void RestoreVertIndices(MeshT const &base, std::vector<Vertex> const &baseVerts,
    ...
    for (uint32_t f = 0; f < numFaces; ++f) {
        int n = FaceDegree(base.polygons[f]);
        for (int k = 0; k < n; ++k)
            vertFaces[base.polygons[f][k]].push_back(f);
    }
    ...
    for (uint32_t f0 = 0; f0 < numFaces; ++f0) {
        ...
        int n = FaceDegree(base.polygons[f0]);
    ...
    dest.polygons = base.polygons;  // indices now aligned; base's polys are valid for dest too
}

void NormalizeObject(Object &dest, Object const &base) {
    if (dest.firstMesh().IsTriangulated())
...
void ConvertSkinning(Object const &base, Object &dest) {
    ...
    for (size_t q = 0; q < base.firstMesh().polygons.size(); q++) {
        for (size_t v = 0; v < base.firstMesh().polygons[q].size(); v++) {
            for (size_t i = 0; i < 8; i++) {
                dest.vertices[dest.firstMesh().polygons[q][v]].boneIndices[i] =
                    base.vertices[base.firstMesh().polygons[q][v]].boneIndices[i];
                dest.vertices[dest.firstMesh().polygons[q][v]].boneWeights[i] =
                    base.vertices[base.firstMesh().polygons[q][v]].boneWeights[i];
            }
        }
    }
...
DeformationData GetDeformationData(Model const &base, Model const &deformed, Object const &source) {
    ...
    if (headBase.firstMesh().polygons.size() != headDeformed.firstMesh().polygons.size() ||
        headBase.firstMesh().polygons.size() != source.firstMesh().polygons.size())
    {
        ::Error("GetDeformationData: Poly count doesn't match (%d/%d/%d)",
            headBase.firstMesh().polygons.size(), headDeformed.firstMesh().polygons.size(), source.firstMesh().polygons.size());
        return data;
    }
    int ExpectedPolyCount = 3160;
    if (headBase.firstMesh().polygons.size() != ExpectedPolyCount) {
        ::Error("GetDeformationData: Poly count is not %d", ExpectedPolyCount);
        return data;
    }
    if (headBase.firstMesh().polygons.back().size() != 3) {
        ::Error("GetDeformationData: Base poly is not a triangle in base object", ExpectedPolyCount);
        return data;
    }
    if (headDeformed.firstMesh().polygons.back().size() != 3) {
    ...
    size_t BaseVertexID = headBase.firstMesh().polygons.back()[2];
