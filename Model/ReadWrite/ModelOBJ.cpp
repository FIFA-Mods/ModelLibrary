...
void Model::ReadObj(std::filesystem::path const &filename, ModelOptions const &options) {
    ...
    struct FaceGroup {
        std::string material;
        std::vector<std::array<uint32_t, 4>> polygons;
    };
    struct ObjGroup {
        std::string name;
    ...
    while (std::getline(file, line)) {
        ...
        if (token == "v") {
            ...
            if (options.AllowQuads && fvTokens.size() == 4) {
                // Keep as a genuine quad, preserving winding order.
                uint32_t i0 = ResolveCorner(fvTokens[0]);
                uint32_t i1 = ResolveCorner(fvTokens[1]);
                uint32_t i2 = ResolveCorner(fvTokens[2]);
                uint32_t i3 = ResolveCorner(fvTokens[3]);
                fg->polygons.push_back({ i0, i1, i2, i3 });
            }
            else {
                // Triangle (size==3), or fan-triangulate anything else (quads when
                // AllowQuads is off, and any 5+ vertex N-gon regardless of AllowQuads).
                uint32_t i0 = ResolveCorner(fvTokens[0]);
                for (size_t vi = 1; vi + 1 < fvTokens.size(); ++vi) {
                    uint32_t i1 = ResolveCorner(fvTokens[vi]);
                    uint32_t i2 = ResolveCorner(fvTokens[vi + 1]);
                    fg->polygons.push_back({ i0, i1, i2, i2 });
                }
            }
        }
    }
    for (auto &og : objGroups) {
        ...
        for (auto &fg : og.groups) {
            ...
            if (options.AllowQuads) {
                for (auto const &poly : fg.polygons) {
                    if (poly[2] != poly[3]) { hasQuads = true; break; }
                }
            }
            if (hasQuads) {
                m.quads = std::move(fg.polygons);
            }
            else {
                m.triangles.resize(fg.polygons.size());
                for (size_t pi = 0; pi < fg.polygons.size(); ++pi)
                    m.triangles[pi] = { fg.polygons[pi][0], fg.polygons[pi][1], fg.polygons[pi][2] };
            }
...
void Model::WriteObj(std::filesystem::path const &filename) {
    ...
    for (auto &obj : objects) {
        ...
        for (auto &mesh : obj.meshes) {
            if (mesh.triangles.empty() && mesh.quads.empty()) continue;
