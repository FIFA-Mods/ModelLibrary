...
std::string MakeUniqueObjectName(std::string const &objName, std::set<std::string> &usedNodeNames) {
...
std::string GetMaterialNameForMesh(FbxMesh *fbxMesh, int materialIndex, std::map<FbxSurfaceMaterial *,
    std::string> &materialToName) {
...
Object &CreateObject(Model &model, std::string const &objName, std::string const &parentName, FbxAMatrix const &transform,
    std::set<std::string> &usedNodeNames, FbxNode *node = nullptr) {
    Object &obj = model.objects.emplace_back();
    obj.name = MakeUniqueObjectName(objName, usedNodeNames);
...
// Quantization precision used ONLY for the Direct+ByPolygonVertex fallback
// (no FBX index exists in that case, so exact value comparison is the only
// option). 1e5 gives ~5 decimal digits of precision - far tighter than any
// meaningful difference in normal/tangent/binormal/uv/color data, while still
// merging corners whose values differ only by floating point noise.
constexpr double kAttribQuantScale = 100000.0;

int64_t QuantizeComponent(double v) noexcept {
    return (int64_t)std::llround(v * kAttribQuantScale);
}

// Sentinel written into the unused components of an "indexed" key slot.
// Real quantized values stay in a small bounded range (attribute magnitude *
// 1e5), so this can never collide with one - it only ever matches itself,
// which happens only when the same real FBX index was used (component 0
// already establishes that).
static constexpr int64_t kIndexKeySentinel = std::numeric_limits<int64_t>::min();

struct PVKey {
    int cp = -1;
    int64_t normal[3] = { 0, 0, 0 };
    int64_t tangent[3] = { 0, 0, 0 };
    int64_t binormal[3] = { 0, 0, 0 };
    int64_t uv[8][2] = {};
    int64_t col[8][4] = {};

    bool operator==(PVKey const &o) const noexcept {
        if (cp != o.cp) return false;
        for (int i = 0; i < 3; ++i) if (normal[i] != o.normal[i]) return false;
        for (int i = 0; i < 3; ++i) if (tangent[i] != o.tangent[i]) return false;
        for (int i = 0; i < 3; ++i) if (binormal[i] != o.binormal[i]) return false;
        for (int i = 0; i < 8; ++i)
            for (int j = 0; j < 2; ++j)
                if (uv[i][j] != o.uv[i][j]) return false;
        for (int i = 0; i < 8; ++i)
            for (int j = 0; j < 4; ++j)
                if (col[i][j] != o.col[i][j]) return false;
        return true;
    }
};
...
struct PVKeyHash {
    size_t operator()(PVKey const &k) const noexcept {
        ...
        for (int i = 0; i < 3; ++i) mix(k.normal[i]);
        for (int i = 0; i < 3; ++i) mix(k.tangent[i]);
        for (int i = 0; i < 3; ++i) mix(k.binormal[i]);
        for (int i = 0; i < 8; ++i) { mix(k.uv[i][0]); mix(k.uv[i][1]); }
        for (int i = 0; i < 8; ++i) { mix(k.col[i][0]); mix(k.col[i][1]); mix(k.col[i][2]); mix(k.col[i][3]); }
        return h;
    }
};

void FbxNodeCallback(Model &model, FbxNode *node, std::string const &parentName, std::set<std::string> &usedNodeNames,
    std::set<FbxNode *> &bonesToInclude, std::map<std::string, uint16_t> &globalBoneIndex, std::map<FbxSurfaceMaterial *,
    std::string> &materialToName) {
    if (!node) return;
    ...
    if (bonesToInclude.find(node) == bonesToInclude.end()) {
        ...
        Object &nodeObj = CreateObject(model, nodeName, parentName, localTransform, usedNodeNames, node);
        ...
        for (int attrIndex = 0; attrIndex < node->GetNodeAttributeCount(); ++attrIndex) {
            ...
            if (meshAttrCount > 1) {
                ...
                objPtr = &CreateObject(model, meshName, newParent, FbxAMatrix(), usedNodeNames); // identity transform
            }
            ...
            std::map<int, std::vector<std::vector<uint32_t>>> matPolys;
            ...
            for (int p = 0; p < polyCount; ++p) {
                ...
                std::vector<uint32_t> polyIdx(polySize);
                ...
                for (int pv = 0; pv < polySize; ++pv) {
                    ...
                    Vector3 cornerNormal(0, 0, 0), cornerTangent(0, 0, 0), cornerBinormal(0, 0, 0);

                    if (nElem) {
                        bool byCP = nElem->GetMappingMode() == FbxGeometryElement::eByControlPoint;
                        bool indexToDirect = nElem->GetReferenceMode() == FbxLayerElement::eIndexToDirect;
                        bool indexed = byCP || indexToDirect;
                        int dIndex = byCP ? cp : (indexToDirect ? nElem->GetIndexArray().GetAt(basePV + pv) : basePV + pv);
                        if (dIndex >= 0 && dIndex < nElem->GetDirectArray().GetCount()) {
                            FbxVector4 nn = nElem->GetDirectArray().GetAt(dIndex);
                            cornerNormal = Vector3((float)nn[0], (float)nn[1], (float)nn[2]);
                        }
                        else {
                            FbxVector4 nn;
                            if (fbxMesh->GetPolygonVertexNormal(p, pv, nn))
                                cornerNormal = Vector3((float)nn[0], (float)nn[1], (float)nn[2]);
                            indexed = false; // fell back to a computed value with no reliable index behind it
                        }
                        if (indexed) {
                            key.normal[0] = dIndex;
                            key.normal[1] = kIndexKeySentinel;
                            key.normal[2] = kIndexKeySentinel;
                        }
                        else {
                            key.normal[0] = QuantizeComponent(cornerNormal.x);
                            key.normal[1] = QuantizeComponent(cornerNormal.y);
                            key.normal[2] = QuantizeComponent(cornerNormal.z);
                        }
                    }

                    if (tElem) {
                        bool byCP = tElem->GetMappingMode() == FbxGeometryElement::eByControlPoint;
                        bool indexToDirect = tElem->GetReferenceMode() == FbxLayerElement::eIndexToDirect;
                        bool indexed = byCP || indexToDirect;
                        int dIndex = byCP ? cp : (indexToDirect ? tElem->GetIndexArray().GetAt(basePV + pv) : basePV + pv);
                        if (dIndex >= 0 && dIndex < tElem->GetDirectArray().GetCount()) {
                            FbxVector4 tt = tElem->GetDirectArray().GetAt(dIndex);
                            cornerTangent = Vector3((float)tt[0], (float)tt[1], (float)tt[2]);
                        }
                        else {
                            indexed = false;
                        }
                        if (indexed) {
                            key.tangent[0] = dIndex;
                            key.tangent[1] = kIndexKeySentinel;
                            key.tangent[2] = kIndexKeySentinel;
                        }
                        else {
                            key.tangent[0] = QuantizeComponent(cornerTangent.x);
                            key.tangent[1] = QuantizeComponent(cornerTangent.y);
                            key.tangent[2] = QuantizeComponent(cornerTangent.z);
                        }
                    }

                    if (bElem) {
                        bool byCP = bElem->GetMappingMode() == FbxGeometryElement::eByControlPoint;
                        bool indexToDirect = bElem->GetReferenceMode() == FbxLayerElement::eIndexToDirect;
                        bool indexed = byCP || indexToDirect;
                        int dIndex = byCP ? cp : (indexToDirect ? bElem->GetIndexArray().GetAt(basePV + pv) : basePV + pv);
                        if (dIndex >= 0 && dIndex < bElem->GetDirectArray().GetCount()) {
                            FbxVector4 bb = bElem->GetDirectArray().GetAt(dIndex);
                            cornerBinormal = Vector3((float)bb[0], (float)bb[1], (float)bb[2]);
                        }
                        else {
                            indexed = false;
                        }
                        if (indexed) {
                            key.binormal[0] = dIndex;
                            key.binormal[1] = kIndexKeySentinel;
                            key.binormal[2] = kIndexKeySentinel;
                        }
                        else {
                            key.binormal[0] = QuantizeComponent(cornerBinormal.x);
                            key.binormal[1] = QuantizeComponent(cornerBinormal.y);
                            key.binormal[2] = QuantizeComponent(cornerBinormal.z);
                        }
                    }
                    ...
                    for (int u = 0; u < nUV; ++u) {
                        ...
                        if (!uvE) continue;
                        bool byCP = uvE->GetMappingMode() == FbxGeometryElement::eByControlPoint;
                        bool indexToDirect = uvE->GetReferenceMode() == FbxLayerElement::eIndexToDirect;
                        bool indexed = byCP || indexToDirect;
                        int dIndex = byCP ? cp : (indexToDirect ? uvE->GetIndexArray().GetAt(basePV + pv) : basePV + pv);
                        if (dIndex >= 0 && dIndex < uvE->GetDirectArray().GetCount()) {
                            FbxVector2 uvt = uvE->GetDirectArray().GetAt(dIndex);
                            cornerUV[u] = Vector2((float)uvt[0], (float)uvt[1]);
                        }
                        else {
                        ...
                        if (indexed) {
                            key.uv[u][0] = dIndex;
                            key.uv[u][1] = kIndexKeySentinel;
                        }
                        else {
                            key.uv[u][0] = QuantizeComponent(cornerUV[u].x);
                            key.uv[u][1] = QuantizeComponent(cornerUV[u].y);
                        }
                    }
                    ...
                    for (int c = 0; c < nCol; ++c) {
                        ...
                        if (!colE) continue;
                        bool byCP = colE->GetMappingMode() == FbxGeometryElement::eByControlPoint;
                        bool indexToDirect = colE->GetReferenceMode() == FbxLayerElement::eIndexToDirect;
                        bool indexed = byCP || indexToDirect;
                        int dIndex = byCP ? cp : (indexToDirect ? colE->GetIndexArray().GetAt(basePV + pv) : basePV + pv);
                        if (dIndex >= 0 && dIndex < colE->GetDirectArray().GetCount()) {
                            FbxColor fc = colE->GetDirectArray().GetAt(dIndex);
                            cornerCol[c][0] = (float)fc.mRed;
                            cornerCol[c][1] = (float)fc.mGreen;
                            cornerCol[c][2] = (float)fc.mBlue;
                            cornerCol[c][3] = (float)fc.mAlpha;
                        }
                        else {
                            indexed = false;
                        }
                        if (indexed) {
                            key.col[c][0] = dIndex;
                            key.col[c][1] = kIndexKeySentinel;
                            key.col[c][2] = kIndexKeySentinel;
                            key.col[c][3] = kIndexKeySentinel;
                        }
                        else {
                            key.col[c][0] = QuantizeComponent(cornerCol[c][0]);
                            key.col[c][1] = QuantizeComponent(cornerCol[c][1]);
                            key.col[c][2] = QuantizeComponent(cornerCol[c][2]);
                            key.col[c][3] = QuantizeComponent(cornerCol[c][3]);
                        }
                    }
                    ...
                    if (it != keyToIndex.end()) {
                        ...
                        V.normal = cornerNormal;
                        V.tangent = cornerTangent;
                        V.binormal = cornerBinormal;
                        for (int u = 0; u < nUV; ++u)
                            V.uv[u] = cornerUV[u];
                        for (int c = 0; c < nCol; ++c)
                            V.colors[c].Set(cornerCol[c][0], cornerCol[c][1], cornerCol[c][2], cornerCol[c][3]);
                ...
                matPolys[matIdx].push_back(polyIdx);
            } // polygon loop
            ...
            for (int bsDef = 0; bsDef < fbxMesh->GetDeformerCount(FbxDeformer::eBlendShape); ++bsDef) {
                ...
                for (int chIdx = 0; chIdx < blendShape->GetBlendShapeChannelCount(); ++chIdx) {
                    ...
                    // targets at partial weights are ignored
            ...
            for (auto &[idx, polys] : matPolys) {
                Mesh m;
                m.material = GetMaterialNameForMesh(fbxMesh, idx, materialToName);
                m.polygons = std::move(polys);
                objPtr->meshes.push_back(std::move(m));
            }
        } // attribute loop
    }
    for (int i = 0; i < node->GetChildCount(); ++i)
        FbxNodeCallback(model, node->GetChild(i), newParent, usedNodeNames, bonesToInclude, globalBoneIndex, materialToName);
}

struct FbxGeometryInfo {
    bool bHasMeshesWithNormals = false;
    bool bHasMeshesWithTangents = false;
    bool bHasMeshesWithoutNormals = false;
    bool bHasMeshesWithoutTangents = false;
    bool bHasNonTriMeshes = false;
};

void GetFbxGeometryInfo(FbxNode *node, FbxGeometryInfo &info) {
    if (!node)
        return;
    for (int attrIndex = 0; attrIndex < node->GetNodeAttributeCount(); ++attrIndex) {
        FbxNodeAttribute *attr = node->GetNodeAttributeByIndex(attrIndex);
        ...
        if (attr->GetAttributeType() != FbxNodeAttribute::eMesh)
            continue;
        FbxMesh *mesh = FbxCast<FbxMesh>(attr);
        if (!mesh)
            continue;
        bool hasNormals = mesh->GetElementNormalCount() != 0;
        bool hasTangents = mesh->GetElementTangentCount() != 0 && mesh->GetElementBinormalCount() != 0;
        if (!info.bHasMeshesWithNormals && hasNormals)
            info.bHasMeshesWithNormals = true;
        if (!info.bHasMeshesWithTangents && hasTangents)
            info.bHasMeshesWithTangents = true;
        if (!info.bHasMeshesWithoutNormals && !hasNormals)
            info.bHasMeshesWithoutNormals = true;
        if (!info.bHasMeshesWithoutTangents && !hasTangents)
            info.bHasMeshesWithoutTangents = true;
        if (!info.bHasNonTriMeshes && !mesh->IsTriangleMesh())
            info.bHasNonTriMeshes = true;
    }
    for (int i = 0; i < node->GetChildCount(); ++i)
        GetFbxGeometryInfo(node->GetChild(i), info);
}
    ...
    ModelReadOptions::eFbxTangentsOption tangents, int UVIndex)
{
    ...
    for (int attrIndex = 0; attrIndex < node->GetNodeAttributeCount(); ++attrIndex) {
        ...
        if (!mesh)
            continue;
        if (normals != ModelReadOptions::FBX_NORMALS_DO_NOTHING)
            mesh->GenerateNormals(normals == ModelReadOptions::FBX_NORMALS_REGENERATE, true, true);
        if (tangents != ModelReadOptions::FBX_TANGENTS_DO_NOTHING)
            mesh->GenerateTangentsData(UVIndex, tangents == ModelReadOptions::FBX_TANGENTS_REGENERATE);
    }
    int childCount = node->GetChildCount();
    for (int i = 0; i < childCount; ++i)
        GenerateNormalsAndTangents(node->GetChild(i), normals, tangents, UVIndex);
}
...
void Model::ReadFbx(std::filesystem::path const &filename, ModelReadOptions const &options) {
    using namespace fbx_helper::fbx_utils;
    Clear();
    SceneFile sceneFile;
    sceneFile.sdkManager = FbxManager::Create();
    if (!sceneFile.sdkManager)
        throw std::runtime_error("unable to create sdk manager");
    sceneFile.ios = FbxIOSettings::Create(sceneFile.sdkManager, IOSROOT);
    sceneFile.sdkManager->SetIOSettings(sceneFile.ios);
    sceneFile.importer = FbxImporter::Create(sceneFile.sdkManager, "");
    if (!sceneFile.importer->Initialize(filename.string().c_str(), -1, sceneFile.sdkManager->GetIOSettings()))
        throw std::runtime_error("unable to initialize importer");
    FbxScene *scene = FbxScene::Create(sceneFile.sdkManager, "scene");
    if (!scene)
        throw std::runtime_error("unable to create scene");
    if (!sceneFile.importer->Import(scene))
        throw std::runtime_error("unable to import scene");
    sceneFile.importer->Destroy();
    sceneFile.importer = nullptr;
    FbxNode *root = scene->GetRootNode();
    if (!root)
        throw std::runtime_error("unable to find scene root node");
    bool triangulate = options.AlwaysTriangulate;
    auto fbxNormals = options.FbxNormals;
    auto fbxTangents = options.FbxTangents;
    if (fbxNormals != ModelReadOptions::FBX_NORMALS_DO_NOTHING || fbxTangents != ModelReadOptions::FBX_TANGENTS_DO_NOTHING) {
        FbxGeometryInfo geomInfo;
        GetFbxGeometryInfo(root, geomInfo);
        // Skip normals generation if nothing is missing
        if (fbxNormals == ModelReadOptions::FBX_NORMALS_GENERATE && !geomInfo.bHasMeshesWithoutNormals)
            fbxNormals = ModelReadOptions::FBX_NORMALS_DO_NOTHING;
        // Edge case: regenerating normals invalidates existing tangents
        if (fbxNormals == ModelReadOptions::FBX_NORMALS_REGENERATE && geomInfo.bHasMeshesWithTangents)
            fbxTangents = ModelReadOptions::FBX_TANGENTS_REGENERATE;
        // Skip tangents generation if nothing is missing
        if (fbxTangents == ModelReadOptions::FBX_TANGENTS_GENERATE && !geomInfo.bHasMeshesWithoutTangents)
        ...
        if (fbxTangents != ModelReadOptions::FBX_TANGENTS_DO_NOTHING && geomInfo.bHasNonTriMeshes)
            triangulate = true;
    }
    if (triangulate) {
        FbxGeometryConverter geomConv(sceneFile.sdkManager);
        if (!geomConv.Triangulate(scene, true))
            throw std::runtime_error("unable to triangulate FBX scene");
    }
    if (fbxNormals != ModelReadOptions::FBX_NORMALS_DO_NOTHING || fbxTangents != ModelReadOptions::FBX_TANGENTS_DO_NOTHING)
        GenerateNormalsAndTangents(root, fbxNormals, fbxTangents, options.TangentsUVIndex);
    ...
    for (int c = 0; c < root->GetChildCount(); ++c)
        FbxNodeCallback(*this, root->GetChild(c), "", usedNodeNames, bonesToInclude, globalBoneIndex, materialToName);
    ModelPostLoadProcess(*this, options);
}

void Model::WriteFbx(std::filesystem::path const &filename, ModelWriteOptions const &options) {
    using namespace fbx_helper::fbx_utils;
